...
soho_dataset = SOHODataset("/gss/r.jarolim/data/soho/train", base_names=['2007-01-01T14:19.fits'])
...
sdo_img[0, 4] = np.abs(sdo_img[0, 4])
...
for c in range(5):
    ...
    #s_map = s_map.submap(SkyCoord(sub_frame_x, sub_frame_y, frame=s_map.coordinate_frame))
    norm = Normalize(vmin=-.5, vmax=.5) if c == 4 else Normalize(vmin=-1, vmax=1)
    s_map.plot(axes=axs[2, c], cmap=sdo_cmaps[c], norm=norm, title='')
