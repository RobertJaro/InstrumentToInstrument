...
from iti.data.align import correlation_coefficient
...
for basename in basenames_soho:
    print(Map('%s/%s/%s' % (soho_data_path, dir, basename)).meta['quality'])
...
for i, (soho_cube, iti_cube, sdo_cube) in tqdm(enumerate(zip(soho_maps, iti_maps, sdo_maps)), total=len(selected_dates)):
    ...
    if (i + 1) % 5 == 0:
        ...
        plt.imsave(os.path.join(prediction_path, '%s_MDI.jpg' % date.isoformat('T')), mdi_map.data, cmap=cmap, vmin=-1500.0, vmax=1500.0, origin='lower')
        plt.imsave(os.path.join(prediction_path, '%s_ITI.jpg' % date.isoformat('T')), iti_map.data, cmap=cmap, vmin=-1500.0, vmax=1500.0, origin='lower')
        plt.imsave(os.path.join(prediction_path, '%s_HMI.jpg' % date.isoformat('T')), hmi_map.data, cmap=cmap, vmin=-1500.0, vmax=1500.0, origin='lower')
        gc.collect()
